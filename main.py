...
def Mochila(poblacion):
    ...
    for i in range(generaciones):  # Ciclo de generaciones
        ...
        poblacion = Remplazo(poblacion, tam_mochila, objetos_mochila, tam_poblacion, prob_mutacion)
    ...
    for individuos in poblacion:                                # Evaluamos la poblacion final
        p_i = FuncionEvaluacion(individuos, objetos_mochila)    # Obtenemos el peso de cada individuo
        pesos_to.append(p_i)
        obj = individuos.count(1)                               # Contamos sus objetos
        ob_total.append(obj)
        mejor_individuo.append((individuos, p_i, obj))
    mejor_individuo.sort(key=lambda x: (x[1], x[2]), reverse=True)      # Ordenamos la lista de mayor peso a menor peso

    mejor_individuo = mejor_individuo[0]                        # Elegimos el primer individuo que es el de mayor peso
                                                                # y mayor número de objetos

    medi, med, des = Estadisticas(pesos_to, ob_total)           # Obtenemos las estadisticas de la poblacion final
...
tam_poblacion = int(input("Ingrese el tamaño de la poblacion: "))                           # Tamaño de la poblacion
generaciones = int(input("Ingrese el numero de generaciones: "))
prob_mutacion = float(input("Ingrese la probabilidad de mutacion(en rango de 0-1): "))      # Probabilidad de mutacion
...
objetos = [(f"Objeto {i}", round(random.uniform(0, 10), 1)) for i in range(num_objetos)]  # Creamos los objetos
...
for i in range(tam_poblacion):
    individuo = [random.randint(0, 1) for i in range(len(objetos))]          # Creamos un individuo con 0 y 1 aleatorios
